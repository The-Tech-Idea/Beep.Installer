...
public class ClickOnceTests
{
    ...
    public void Stage_ProducesDeployFiles_AndManifests()
    {
        ...
        try
        {
            var r = PublishStager.Stage(payload, publish, "MyApp", "1.0.0", "Pub", "An app", exe, "https://host/MyApp.application");
            ...
            Directory.GetFiles(appDir, "MyApp.manifest", SearchOption.TopDirectoryOnly).Should().HaveCount(1);
    ...
    public void ApplicationManifest_ListsEveryFile_WithCorrectSha256()
    {
        ...
        try
        {
            var r = PublishStager.Stage(payload, publish, "MyApp", "1.0.0", null, null, exe, null);
    ...
    public void DeploymentManifest_ReferencesAppManifest_WithMatchingDigest()
    {
        ...
        try
        {
            ...
            var r = PublishStager.Stage(payload, publish, "MyApp", "1.0.0", "Pub", "d", exe, updateUrl);

            var doc = XDocument.Load(r.DeploymentManifestPath);
            var dep = doc.Root!.Element(Asm + "dependency")!.Element(Asm + "dependentAssembly")!;
            dep.Attribute("codebase")!.Value.Should().Be("Application/MyApp.manifest");
