...
public class SilentFailureGuardTests
{
    ...
    public void CoreLibrary_DoesNotBlockOnAsync()
    {
        ...
        // stays visible:
        //   MageManifestTool — drains two pipe reads that WaitForExit has already completed, on
        //     the build path, never on the installer's UI thread.
        // (The ClickOnce UpdateChecker/UpdateApplier pair was retired in P11.C — superseded by the
        //  async-from-day-one BeepDM Updates domain — and IInstallerHostBuilder no longer blocks,
        //  so both of those exemptions are gone.)
        var exempt = new[] { "MageManifestTool.cs" };
