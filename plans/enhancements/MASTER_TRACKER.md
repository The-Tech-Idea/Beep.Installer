...
**P12 complete through 12.D.4.** 12.C.2 (`1e17c64`): the concrete-duplication check found
Prerequisites/Shortcuts/Registry hand-rolled the same wiring `BuildAdvancedResourceSection<T>`
already provides for the 17 typed-resource sections — routed through it directly, removing ~90
lines and 9 dead fields, and fixing two latent bugs (unhidden list-typed grid columns, unhandled
`DataError`) as a side effect. 12.D.4 (`4b767bb`): Next now refuses to leave the Components step
empty — the one concrete per-step rule that couldn't come from field-level validation, since a
`DataGridView`'s row count isn't a named control. Only 12.E remains, explicitly opportunistic and
unbounded by design (apply the shared helpers to the remaining ~28 advanced-only sections as they
are next touched for any other reason) — not a blocker, not scheduled as a sweep.

Full suite: 1496 passed, 0 failed, 4 skipped, 2m41s (environment note: a pile of stray `dotnet`/
`testhost` processes — some hours old — had been silently contending for CPU/disk for much of this
session, making every build and test run far slower than normal; clearing them brought a full run
back down from 11 minutes to under 3).
...
| 12 | Wizard-first IA + shared editing ViewModel | P1 | ✅ 12.A–12.D.4 shipped; 12.E opportunistic | [P12](P12_WIZARD_FIRST_IA_DESIGN.md) |
