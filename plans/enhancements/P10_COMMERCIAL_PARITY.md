...
|---|------|-------|--------|--------|
| 10.A.1a | Make `UpgradeEngine` scope-aware: `DetectExisting`/`RegisterInstall` require an explicit hive; `UnregisterInstall` and `RegistrationKeyPath` use the same product-owned key path | `BeepDM/.../Installer/UpgradeEngine.cs` | `RegisterAndDetect_RoundTrip_InTheGivenHive` (HKCU) | ✅ |
...
| 10.A.2b | `CommitUpgradeStep` terminal (after verify — backup discarded only once install proven): restore/merge user config from backup then delete it; failure path: both hosts (silent + wizard UI) call `RestoreFromBackup` when a backup was recorded | `UpgradeStep.cs` (both steps); `Program.cs`; `BeepModernInstallerForm.cs` | commit/restore tests | ✅ |
